...
#Find LMC Analogs
print('Searching for LMC Analogs...')
if args.definition == '1':
    criteria = mvir
    ##previous (lb,ub) values: (5e11,3e12),(1e12,4e12)
    lower_bound_LMC = 1e11  
    upper_bound_LMC = 2.5e11
else:
    criteria = mstar
    ##previous (lb,ub) values: (10**10.2,10**10.9)
    lower_bound_LMC = 2.2**9 #if args.definition == '4' else 10**10.2
    upper_bound_LMC = 3.2**9 #if args.definition == '4' else 10**10.9
...
for i in np.arange(len(hnum)):
    if lower_bound_LMC < criteria[i] < upper_bound_LMC:
        LMCs[str(hnum[i])] = {}
        LMCs[str(hnum[i])]['Mvir'] = mvir[i]
        LMCs[str(hnum[i])]['Mstar'] = mstar[i]
        LMCs[str(hnum[i])]['Rvir'] = rvir[i]
        LMCs[str(hnum[i])]['Vmag'] = vmag[i]
        LMCs[str(hnum[i])]['Rmag'] = rmag[i]
        LMCs[str(hnum[i])]['Kmag'] = kmag[i]
        LMCs[str(hnum[i])]['Bmag'] = bmag[i]
        LMCs[str(hnum[i])]['Gmag'] = gband(bmag[i],vmag[i])
        LMCs[str(hnum[i])]['center'] = cen[i]
        LMCs[str(hnum[i])]['CumSFH'] = csfh[i]
        LMCs[str(hnum[i])]['SFR_250Myr'] = sfr[i]
        LMCs[str(hnum[i])]['Satellites'] = [] #Empty array for indexes of satellites of this MW
myprint(f'{len(LMCs)} LMC Analogs Found',clear=True)
...
#Remove any LMC Analogs with overlapping Virial Radii
overlapping_LMC = []
for mw1 in LMCs:
    for mw2 in LMCs:
        if not mw1 == mw2:
            distance = LMCs[mw1]['center'] - LMCs[mw2]['center']
            wrap(distance)
            if np.linalg.norm(distance) < ( (LMCs[mw1]['Rvir']+LMCs[mw1]['Rvir']) if args.radius=='sim' else 400):
                if mw1 not in overlapping_LMC:
                    overlapping_LMC.append(mw1)
                if mw2 not in overlapping_LMC:
                    overlapping_LMC.append(mw2)
for mw in overlapping_LMC:
    del LMCs[mw]
print(f'\t{len(overlapping_LMC)} LMCs removed due to overlapping Rvir')#: {overlapping}')


#remove any LMC Analogs within virial radius of a MW+
close_LMC = []
for gal1 in LMCs:
    for gal2 in MilkyWays:
        if not gal1 == gal2:
            distance = LMCs[gal1]['center'] - MilkyWays[gal2]['center']
            wrap(distance)
            if np.linalg.norm(distance) < ( (MilkyWays[gal2]['Rvir']+MilkyWays[gal2]['Rvir']) if args.radius=='sim' else 600):
                if gal1 not in close_LMC:
                    close_LMC.append(gal1)
                    
for gal in close_LMC:
    del LMCs[gal]
print(f'\t{len(close_LMC)} LMCs removed due to close proximity to Milky Way')#: {overlapping}')
...
#Determine Closest MW+ and Satellites for LMC Analogs
print(f'Finding Satellite Halos for LMCs...')
for mw in LMCs:
    rad = 200 if args.radius=='300' else LMCs[mw]['Rvir']
    mw_plus_id,mw_plus_dist = [[],[]]
    for i in np.arange(len(hnum)):
        if mstar[i] < 1e7 or rmag[i] > -10.8 or str(hnum[i])==mw:
            pass #Outside Rom25 resolution limit or outside SAGA detection limit
        else:   #Satellite Halos
            distance = LMCs[mw]['center'] - cen[i]
            wrap(distance)
            if np.linalg.norm(distance) < rad:
                if str(hnum[i]) in LMCSats:
                    sys.exit(f'Satellite {hnum[i]} has multiple hosts!!!')
                if criteria[i] < LMCs[mw]['Mvir']:    
                    LMCSats[str(hnum[i])] = {}
                    LMCSats[str(hnum[i])]['Mvir'] = mvir[i]
                    LMCSats[str(hnum[i])]['Rvir'] = rvir[i]
                    LMCSats[str(hnum[i])]['Vmag'] = vmag[i]
                    LMCSats[str(hnum[i])]['Kmag'] = kmag[i]
                    LMCSats[str(hnum[i])]['Bmag'] = bmag[i]
                    LMCSats[str(hnum[i])]['Rmag'] = rmag[i]
                    LMCSats[str(hnum[i])]['Gmag'] = gband(bmag[i],vmag[i])
                    LMCSats[str(hnum[i])]['Mstar'] = mstar[i]
                    LMCSats[str(hnum[i])]['center'] = cen[i]
                    LMCSats[str(hnum[i])]['CumSFH'] = csfh[i]
                    LMCSats[str(hnum[i])]['SFR_250Myr'] = sfr[i]
                    LMCSats[str(hnum[i])]['Host'] = mw
                    LMCSats[str(hnum[i])]['Orbit'] = [np.linalg.norm(distance),
                                            np.linalg.norm(distance)/LMCs[mw]['Rvir']]
                    
                    LMCs[mw]['Satellites'].append(str(hnum[i]))
                  
            if criteria[i] > lower_bound_LMC: # LMC+ sized halos
                mw_plus_dist.append(np.linalg.norm(distance))
                mw_plus_id.append(str(hnum[i]))
    LMCs[mw]['Closest_MW+'] = [min(mw_plus_dist),
                                    mw_plus_id[mw_plus_dist.index(min(mw_plus_dist))]]
myprint(f'{len(LMCSats)} LMC Satellite Halos Found',clear=True)
...
#Determine LMC Satellite Quenching
for s in LMCSats:
    ######## if t_at_sffrac(satfile[h]['CumSFH'],.99) < 11:
    if float(LMCSats[s]['SFR_250Myr'][-1])/float(LMCSats[s]['Mstar']) < 1e-11:   
        LMCSats[s]['Quenched'] = True
    else:
        LMCSats[s]['Quenched'] = False
...
#Determine Closest Large Halo to Milky Way Analogs and Satellites for LMCs
print('Calculating Closest Large Halo to MW Analogs and Satellites for LMCs...')
for mw in LMCs:
    lh_id,lh_dist = [[],[]]
    for lh in LargeHalos:
        if not mw == lh:
            distance = LMCs[mw]['center'] - LargeHalos[lh]['center']
            wrap(distance)
            lh_dist.append(np.linalg.norm(distance))
            lh_id.append(lh)
    LMCs[mw]['Closest'] = [min(lh_dist),lh_id[lh_dist.index(min(lh_dist))]]
for s in LMCSats:
    lh_id,lh_dist = [[],[]]
    for lh in LargeHalos:
        if not s == lh:
            distance = LMCSats[s]['center'] - LargeHalos[lh]['center']
            wrap(distance)
            lh_dist.append(np.linalg.norm(distance))
            lh_id.append(lh)
    LMCSats[s]['Closest'] = [min(lh_dist),lh_id[lh_dist.index(min(lh_dist))]]
myprint('Closest Large Halos Found',clear=True)
...
#Output Data Files for LMC
print('Creating Data Files...')
out = open(output_path + f'LMCs.{args.definition}.{args.radius}.pickle','wb')
pickle.dump(LMCs,out)
out.close()
out = open(output_path + f'LMCSats.{args.definition}.{args.radius}.pickle','wb')
pickle.dump(LMCSats,out)
out.close()
myprint('LMC Data Files Written',clear=True)
